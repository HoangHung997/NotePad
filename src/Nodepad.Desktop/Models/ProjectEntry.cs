...
public sealed class ProjectEntry : ObservableObject
{
    ...
    public int OrderNumber
    {
        ...
        set
        {
            ...
            {
                OnPropertyChanged(nameof(DisplayOrderLabel));
            }
        }
    }
    ...
    public string DisplayName
    {
        get
        {
            var plainText = RichTextDocumentSerializer.ExtractPlainText(Name).Trim();
            return string.IsNullOrWhiteSpace(plainText) ? "Untitled Project" : plainText;
        }
    }

    [JsonIgnore]
    public string DisplayOrderLabel => $"{Math.Max(1, OrderNumber)}.";

    [JsonIgnore]
    public ChecklistItem? CurrentChecklistItem => ChecklistItems.FirstOrDefault(item =>
        !item.IsCompleted && RichTextDocumentSerializer.HasPlainText(item.Text));
    ...
    public string DisplayNextAction
    {
        get
        {
            ...
            {
                return CurrentChecklistItem.DisplayText;
            }
    ...
    [JsonIgnore]
    public int TotalChecklistCount => ChecklistItems.Count(item => RichTextDocumentSerializer.HasPlainText(item.Text));

    [JsonIgnore]
    public int CompletedChecklistCount => ChecklistItems.Count(item => item.IsCompleted && RichTextDocumentSerializer.HasPlainText(item.Text));
    ...
    public bool Matches(string searchText)
    {
        ...
        return DisplayName.Contains(token, StringComparison.OrdinalIgnoreCase)
               ...
               || ChecklistItems.Any(item => RichTextDocumentSerializer.ExtractPlainText(item.Text)
