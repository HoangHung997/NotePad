...
public sealed class AgentRunner : IDisposable
{
        ...
        Action<string, string> emit, Action persist, CancellationToken ct)
    {
        ...
        for (var step = 0; step < 24; step++)
        {
            ...
            {
                received += line.Length; if (received > 2_000_000) throw new IOException("Phản hồi vượt giới hạn 2 MB của bản thử.");
            ...
            if (calls.Count == 0)
            {
                ...
                session.Add("assistant", content.ToString()); persist(); emit("final", content.ToString()); return;
            }
            ...
            foreach (var call in parsed)
            {
                ...
                var result = blocked is null ? await tools.Execute(call, ct) : new JsonObject
                {
