...
public sealed partial class ChatCompletionsTransport
{
        ...
        AgentTransportContinuationRequest acknowledged, CancellationToken cancellationToken)
    {
        ...
        foreach (var message in context.Messages) input.Add(ToChatMessage(message));
        return _requestBudget.Preview(BuildContextPayload(input, tools),
            _taskId, _turnId, "ChatCompletionsTransport", cancellationToken);
    }
