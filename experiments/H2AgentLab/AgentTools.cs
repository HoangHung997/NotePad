    ...
    : IDisposable
{
    ...
    [
        ...
        Def("inspect_artifact", "Independently reopen a recorded output and persist an exact-byte verification receipt. DOCX/XLSX use host OpenXML readers; PDF/image receipt is signature-only unless separate content/layout evidence exists.", ("run_id", "Recorded runId"), ("path", "Relative path within that run's output directory")),
        ...
        Def("publish_artifact", "Publish a staged output only after inspect_artifact created an exact-byte verification receipt; existing destinations require current read_file hash and get a backup. Signature-only PDF/image output remains unverified for content/layout.", ("run_id", "Recorded runId"), ("path", "Output artifact path"), ("destination", "Workspace-relative target"), ("expected_hash", "Current destination hash; empty for new file")),
        ...
        try
        {
            ...
            switch (call.Name)
            {
                ...
                case "inspect_artifact":
                    var inspection = Scripts.VerifyArtifact(Arg("run_id"), path);
                    result = new { path, sha256 = inspection.Verification.Sha256, characters = inspection.Characters,
                        content = inspection.Content, truncated = inspection.Truncated, verification = inspection.Verification,
                        visualReview = inspection.Verification.LayoutVerified,
                        requiresFurtherVerification = inspection.Verification.RequiresFurtherVerification }; break;
                case "view_artifact":
