...
| AR-024 | Lát cắt Office qua H2 thật | 022/023 | E4 | USER_ACCEPTED_SEQUENCE / IMPLEMENTED / E1-E2_PASS / E4_DEFERRED_BY_USER_AWAITING_ENVIRONMENT — final full build ready; no E4 PASS claim |
...
**AR-024 implementation checkpoint — 2026-09-26:** exact validated code SHA `4e6b4a07f4103edd654b680ca9af7f485f8b04d8`. Two E2 production slices now traverse the real product surfaces rather than `RecordingExecutor`: Global Work Assistant → `H2ProductionAgentAdapter` → runtime/tool session → `H2OfficeRuntimeTools` → separate OfficeHost process for Excel, and Project Agent → the same production bridge/runtime → OfficeHost for Word. The OfficeHost process runs in fixture mode for E2; this proves production wiring, permission/grounding, host verification and process IPC, **not** native Word/Excel E4.

The Global Excel slice deliberately does **not** fabricate HWND/PID/native capture identity for the fixture. It drops the capture chip, uses task-local FullAccess, and requires the user's prompt to ground the exact workbook path; model-supplied `session_id` alone is not authority. The final repair `4e6b4a07...` made that exact path explicit. This preserved the production rule that more permission does not select a target.

Repair history in this AR-024 turn: `58ba74ef...` added the production-slice gate; `6c928442...` fixed the missing Office protocol namespace; `2b3061ab...` removed fake captured-window authority from the Global fixture; `6e53365f...` added explicit Excel verify readback without weakening completion; `472ba1f2...` added bounded completion diagnostics; `f800b4b9...` cancels superseded AR-024 validation runs; `53592771...` makes focused AR-024 failure fail fast; `4e6b4a07...` grounds the exact Excel workbook path and is the final validated code.
...
**AR-041 implementation checkpoint — 2026-09-26:** exact validated code SHA `ca215bf616bca98f94fafa11f8ec0690bc99e961`. AR-041 reuses the existing AR-031 local immutable Agent journal and adds host-owned restart reconciliation; it does **not** add a second recovery database or auto-resume engine. The public H2 adapter exposes `ReconcileInterruptedTask` with typed host observations. Durable operation receipts keep Prepared/Dispatched/Result identity plus reconciliation state/evidence/version metadata; raw executable arguments and raw document observations are not persisted.

Restart behavior is fail-closed: Prepared-only proves the executor was never dispatched and is finalized as `NoEffect`; Dispatched/Unknown stays blocked until the host reobserves the exact resource; an Applied result from an interrupted task remains `AppliedUnverified` until independent verification; wrong resource hash becomes `NeedsUserResourceMismatch`; `CancelOnHostExit` jobs persisted as Running become `NeedsUserWorkerDead` and are never adopted by PID. `RepairRequired` and `NeedsUser` remain blockers, not retry instructions. Terminal Verified/NoEffect receipts are idempotent only for the same resource/version. Reconciliation never executes a tool/model, never replays a mutation, and returns `RequiresFreshPermission=true`; the archived task remains Blocked/Interrupted for AR-052 to resume later under a new permission/resource binding.

Dedicated AR-041 run `36225962252` / job `108359938339` **SUCCESS**: focused AR-041 **6/6**, retained AR-031 **39/39**, AR-040 **57/57**, AR-024 **2/2**, AR-033 **42/42**, AR-022 **8/8**, full H2 **1317/1317**, and all **75** required Agent suites PASS. Evidence artifact `10901281618`, 430,125 bytes, SHA256 `21123b9eae6339366ae67a40f428de9956844f2ced4115458996844b0add4f79`, was independently downloaded; ZIP integrity passed across **647** entries and `validation.json` reports E1=PASS, E2=PASS, E3=DEFERRED_BY_USER_AWAITING_ENVIRONMENT, clean_end=true, replay_claim=false, exactly_once_claim=false and fresh_permission_required=true.

Full Avalonia CI `36225962431` / job `108359938825` **SUCCESS**, including full H2/Agent/MB/Office/Desktop/Web/CAD/MCP/plugin/transport gates, self-contained Windows x64 publish and packaged DesktopHost/OfficeHost startup+IPC. All **17/17** pull-request workflows on exact SHA completed SUCCESS, 0 failed. Final portable artifact `10900553554` is 110,405,384 bytes, SHA256 `26b5bbab87e1fbcb92f1cb845f79b7429172d11bb8a36899e353908c550435ac`; it was independently downloaded, ZIP integrity passed across **482** entries, and `H2Notes.Avalonia.exe`, `H2AgentLab.DesktopHost.exe`, and `H2AgentLab.OfficeHost.exe` are present.
...
**AR-052 implementation checkpoint — 2026-09-26:** exact validated code SHA `a2263e47a804d2f9f386296b1330b8a1c4231e2b`. Resume now reuses the original **TaskId** and durable thread/history, but always starts a **fresh TurnId and fresh provider request** from canonical H2 journal/goal/evidence state; no provider-specific continuation/response ID is copied between models/backends. The active goal state is rehydrated from the durable `H2AgentGoalSnapshot` plus evidence references, preserving verified outcomes and historical mutation revisions without replaying old mutations or old transcript text.

Resume fails closed before provider allocation when AR-041 still reports `ReconcileRequired`. Mutation resume requires a fresh active permission scope; an expired/revoked old grant is never restored. The selected model/profile is resolved explicitly for the resume turn; provider failure does not auto-fallback. A later explicit provider/model rebase is allowed only as another fresh turn on the same TaskId. Durable steering receipts are ACKed after reconnect without enqueuing the same correction twice. Scoped history exposes bounded durable source locators so a smaller-context model can retrieve exact prior facts without converting old history into current authority or verification.

Dedicated AR-052 run `36237795203` / job `108392677499` **SUCCESS**: focused AR-052 **8/8**; retained AR-041 **6/6**, AR-042 **6/6**, AR-051 **37/37**, AR-031 **39/39**, AR-050 **50/50**; full H2 **1331/1331**; all **75/75** required Agent suites PASS. Protocol matrix covers **Ollama / OpenAI Chat Completions / OpenAI Responses** using their real serializer/transport implementations with deterministic local handlers. Evidence artifact `10905007711`, 429,854 bytes, SHA256 `88e026887d8dbb030a34ab3c1bdfd47300f2128e52cd865bff248e42bb6cab66`, was independently downloaded; ZIP integrity passed across **647** entries and `validation.json` reports E1=PASS, E2=PASS, E4=DEFERRED_BY_USER_AWAITING_ENVIRONMENT, clean_end=true, old_provider_continuation_reused=false, auto_provider_fallback=false, mutation_replay_claim=false, fresh_turn_required=true and fresh_permission_for_mutation=true.

Full Avalonia CI `36237795173` / job `108392643192` **SUCCESS**, including full H2/Agent/MB/Office/Desktop/Web/CAD/MCP/plugin/transport gates, self-contained Windows x64 publish and packaged DesktopHost/OfficeHost startup+IPC. All **19/19** pull-request workflow identities on exact code SHA completed SUCCESS, 0 failed. Final portable artifact `10905665137` is 110,421,122 bytes, SHA256 `513e11d60c1f3ce9528aed532255e67a00e6754cc4bd9861c957ee15f597e112`; it was independently downloaded, ZIP integrity passed across **482** entries, and `H2Notes.Avalonia.exe`, `H2AgentLab.DesktopHost.exe`, and `H2AgentLab.OfficeHost.exe` are present.
...
```yaml
{
  ...
  "phase": "AR-052_IMPLEMENTED_E1_E2_PASS_E4_DEFERRED_FINAL_BUILD_READY",
  "active_task": null,
  "parked_task": "AR-052",
