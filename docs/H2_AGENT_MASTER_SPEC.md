...
MB-113 closes the AutoCAD reference-extension gate with `Acceptance/MbAutoCadAcceptanceTests.cs`. The six deterministic cases prove typed v2/fail-closed AutoCAD schemas, structured document/entity discovery with document and entity state tokens, explicit attribute/layer reads, host-owned authorization plus bounded mutation payloads, stale-token rejection after mutation, independent `autocad.verify_entity` verification against current state, and a default public surface with no arbitrary command/script/LISP execution tool. `AutoCadNativeToolExecutor` is the host adapter from ToolRegistry calls to `IAutoCadNativeBridge`; mutation permission is injected by the host authorizer rather than model arguments, while `AutoCadProviderPolicy` bounds structured payload size/depth/property count and rejects arbitrary execution fields.

MB-114 closes the MCP reference-extension gate with `Acceptance/MbMcpAcceptanceTests.cs`. The six deterministic offline cases prove connect/disconnect/reconnect plus transient recovery, normalized tool/resource discovery, deferred selected-schema loading into the provider-neutral ToolRegistry path, host policy enforcement for both tool mutation scopes and resource-read scopes before provider side effects, exact provider/server/tool version provenance in descriptors/evidence without environment-value leakage, and cancellation/timeout behavior. `McpToolProvider` now retains discovered resource scopes and applies `CapabilityProviderPolicy` before `resources/read`; `McpServerConnection` stops the transport and transitions health to `Failed` when the reconnect budget is exhausted, while caller cancellation still propagates without retry.

MB-115 closes the plugin package lifecycle reference-extension gate with `Acceptance/MbPluginPackageLifecycleAcceptanceTests.cs`. Five deterministic end-to-end cases cover all required lifecycle boundaries: staged install plus exact ToolRegistry hot-registration provenance/execution; update blocked while a tool call is in flight and fresh approval for permission deltas; archive SHA-256 mismatch plus declarative self-test fail-closed behavior without disturbing the current active version; explicit rollback restoring both activation metadata and runtime descriptors/executors; and quarantine diagnostics with automatic rollback of an active bad version. The accepted run proves plugin lifecycle remains an extension-system concern implemented through the existing `PluginManager`, rather than adding plugin-specific logic to AgentRuntime.
