...
## Retained CI regression maintenance

The full and focused source-consent runs at `2f2dda5f6207f948ed56e30ca5cd2c50cb811948` passed, but two independently triggered peer workflows remained red. The checkpoint guard `35906863619` refused all documentation writes rather than hide these failures.

AR-033 consistency run `35904844588` failed its historical old-source-control precondition because the assessment had since acquired the legitimate AR-040 job-observation interface. It failed before replacing any source. All unchanged current AR-033 cases passed in each of three repetitions, and the original negative-control evidence was recovered from successful run `35799370449`, artifact `10725382259`, with CRC and SHA256 checked. That archived run has 36 passes and the exact six expected contradiction failures, then clean source restoration. The current `H2AgentCompletionTests.cs` blob is byte-identical to that run (`f50eb5424d9da3116bc09e702fef8333eb19eea6`).

The workflow now checks this immutable executed-control provenance read-only instead of inserting a pre-job-era class into an evolved assembly. It still builds and runs every current AR-033 case three times, verifies the complete 42-case inventory and all six contradiction tests, and runs the existing independent suites. Eight refusal checks reject corrupted historical or current inventories. This is a retained historical negative control, explicitly NOT a newly run source-substitution experiment. No new AR-066 differential experiment was attempted and the earlier tool-safety-blocked result remains unchanged.

AR-040 run `35904844594` failed only one Agent resilience cancellation case. Its prearmed 40 ms timer raced with request serialization/dispatch on a loaded runner; the failed log did not include the actual send count, so the exact scheduling interleaving is not claimed proven. The same suite passed separately on the same source. The fixture now cancels after an observed request/write, with bounded waits and owned task cleanup, retaining exact one-send and zero-fallback assertions. Four new pre-cancelled HTTP/WebSocket cases explicitly require zero sends. The resilience suite now has 17 cases; the 74-suite count is unchanged. No transport/runtime retry behavior or production timeout was changed. The test/CI-only repair is `17af5676bcfb397441a4a8aae264c6088a5701e1`; all acceptance counts must be verified on that SHA before final checkpoint.
